...
def default_values_menu():  # Brings up the menu for changing the default values in the TCDef.dat file
    f = open("TCDef.dat", "r")
    print()
    ...
    # Allows user to only input numbers from 1-7 since there are 7 options in the menu
...
def def_val_menu_num(item_change):  # Accesses the default values in the TCDef.dat file and allows the user
    ...
    # Read all the values in the default values list, assign them to variables, and allow the user to change
    # the value they want.
    f = open("TCDef.dat", "r")
    ...
    claim_amtJAN = float(f.readline())
    claim_amtFEB = float(f.readline())
    claim_amtMAR = float(f.readline())
    claim_amtAPR = float(f.readline())
    claim_amtMAY = float(f.readline())
    claim_amtJUN = float(f.readline())
    claim_amtJUL = float(f.readline())
    claim_amtAUG = float(f.readline())
    claim_amtSEP = float(f.readline())
    claim_amtOCT = float(f.readline())
    claim_amtNOV = float(f.readline())
    claim_amtDEC = float(f.readline())
    f.close()
    if item_change == 1:
        while True:
            try:
...
def write_default_values(): # Writes all the new, changed, edited, etc, default values back to the TCDef.dat file
    ...
    f.write("{}\n".format(str(claim_amtJAN)))
    f.write("{}\n".format(str(claim_amtFEB)))
    f.write("{}\n".format(str(claim_amtMAR)))
    f.write("{}\n".format(str(claim_amtAPR)))
    f.write("{}\n".format(str(claim_amtMAY)))
    f.write("{}\n".format(str(claim_amtJUN)))
    f.write("{}\n".format(str(claim_amtJUL)))
    f.write("{}\n".format(str(claim_amtAUG)))
    f.write("{}\n".format(str(claim_amtSEP)))
    f.write("{}\n".format(str(claim_amtOCT)))
    f.write("{}\n".format(str(claim_amtNOV)))
    f.write("{}\n".format(str(claim_amtDEC)))
    f.close()
...
def write_previous_years_values():  # Writes all the monthly totals from the previous year to the
    ...
    cur_date_year = cur_date.year - 1
    f.write("Travel Claim Totals Per Month for {}\n".format(str(cur_date_year)))
    f.write("January - {}\n".format(str(str_and_pad(claim_amtJAN))))
    f.write("February - {}\n".format(str(str_and_pad(claim_amtFEB))))
    f.write("March - {}\n".format(str(str_and_pad(claim_amtMAR))))
    f.write("April - {}\n".format(str(str_and_pad(claim_amtAPR))))
    f.write("May - {}\n".format(str(str_and_pad(claim_amtMAY))))
    f.write("June - {}\n".format(str(str_and_pad(claim_amtJUN))))
    f.write("July - {}\n".format(str(str_and_pad(claim_amtJUL))))
    f.write("August - {}\n".format(str(str_and_pad(claim_amtAUG))))
    f.write("September - {}\n".format(str(str_and_pad(claim_amtSEP))))
    f.write("October - {}\n".format(str(str_and_pad(claim_amtOCT))))
    f.write("November - {}\n".format(str(str_and_pad(claim_amtNOV))))
    f.write("December - {}\n".format(str(str_and_pad(claim_amtDEC))))
    f.write("\n")
    f.close()
...
def str_and_pad(dollar_value):  # Formats a number to 2 decimal places, aligns it and adds the $ for currency
    dollar_valueStr = "${:,.2f}".format(dollar_value)
    dollar_valuePad = "{:<8}".format(dollar_valueStr)
...
def emp_travel_claim():     # Allows the user to enter a new travel claim
    ...
    while True:
        ...
        if (car_used == "O") or (car_used == "o"):
            mile_amt = MILEAGE_RATE * km_trav
        elif (car_used == "R") or (car_used == "r"):
            mile_amt = RENTAL_CAR_RATE * days_num
...
def print_travel_claim():   # Allows the user to print the travel claim with totals per entry and totals for
    ...
    # Print Headings
    cur_date = datetime.datetime.now()
    claim_date_head = cur_date.strftime("%m-%d-%Y")
    CenterEnd = "END OF REPORT"
    ...
    for claims in f:
        ...
        mile_amtAccPad = str_and_pad(mile_amtAcc)
        per_diemAcc += per_diem
        per_diemAccPad = str_and_pad(per_diemAcc)
        claim_totAcc += claim_tot
        claim_totAccPad = str_and_pad(claim_totAcc)
...
def graph_monthly_claims(): # Allows the user to print a graph showing the monthly amounts for travel claims
    ...
    # Read the default values to prepare them for updating
    f = open("TCDef.dat", "r")
    CLAIM_NUM = int(f.readline())
    HST = float(f.readline())
    LOW_PER_DIEM_RATE = float(f.readline())
    ...
    while True:
        try:
            cur_datestr = int(input("What is the current date (enter 1 for current date or 2 for Jan 1): "))
        except:
            print("Invalid response - please enter 1 or 2!")
        else:
            if cur_datestr == 1:
                cur_datenow = datetime.datetime.now()
                print(cur_datenow.strftime("%Y-%m-%d"))
                print()
                break
            elif cur_datestr == 2:
                cur_datestr = "2021-01-01"
                cur_datenow = datetime.datetime.strptime(cur_datestr, "%Y-%m-%d")
                print(cur_datenow.strftime("%Y-%m-%d"))
                print()
                break

    if cur_datenow.month == 1 and cur_datenow.day == 1:
        write_previous_years_values()
    ...
    if cur_datenow.month == 1 and cur_datenow.day == 1:
        ...
        for StartDate in f:
            ...
            date_start_str = str(ClaimList[4].strip())
            date_start = datetime.datetime.strptime(date_start_str, "%Y-%m-%d")
            claim_amt = str(ClaimList[12].strip())
            claim_amt = float(claim_amt)

            # Pull month from datetime object:
            start_month = date_start.month

            # Calculate the totals for each month based on the entries in the Claims.dat file.
            if start_month == 1:
                claim_amtJAN += claim_amt
            elif start_month == 2:
                claim_amtFEB += claim_amt
            elif start_month == 3:
                claim_amtMAR += claim_amt
            elif start_month == 4:
                claim_amtAPR += claim_amt
            elif start_month == 5:
                claim_amtMAY += claim_amt
            elif start_month == 6:
                claim_amtJUN += claim_amt
            elif start_month == 7:
                claim_amtJUL += claim_amt
            elif start_month == 8:
                claim_amtAUG += claim_amt
            elif start_month == 9:
                claim_amtSEP += claim_amt
            elif start_month == 10:
                claim_amtOCT += claim_amt
            elif start_month == 11:
                claim_amtNOV += claim_amt
            elif start_month == 12:
                claim_amtDEC += claim_amt
        ...
        mpl.ylabel("Claim Totals($)", color="green")
        mpl.title("Total Sales Each Month")

        # Plotting the x and y axis on line graph:
        mpl.plot(x_axis, y_axis, color='darkblue', marker='*', label="item 1")
        mpl.show()
        mpl.savefig("Total Sales each Month.jpg")

        # Write all the default values back to the TCDef.dat file
        write_default_values()

    print()
    print()
    any_key = input("Process completed. Press any key to go back to main menu....")
    print()